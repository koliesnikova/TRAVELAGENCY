...
public class MysqlTourDao implements TourDAO {
    ...
    public Tour save(Tour tour) throws EntityNotFoundException {
        ...
        if (tour.getId() == null) { //insert
            ...
            valuesMap.put("type_tour", tour.getType_tour());
            valuesMap.put("date_begin", tour.getDate_begin());
            valuesMap.put("date_end", tour.getDate_end());
            valuesMap.put("druh_jedla", tour.getDruh_jedla());
            valuesMap.put("hotel", tour.getHotel());
            return new Tour(
            ...
            int changed = jdbcTemplate.update(sql, tour.getType_tour(), tour.getDate_begin(), tour.getDate_end(), tour.getDruh_jedla(), tour.getHotel());
            if (changed == 1) {
                return tour;
            } else {

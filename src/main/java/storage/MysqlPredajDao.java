...
public class MysqlPredajDao implements PredajDAO {
    ...
    public Predaj save(Predaj predaj) throws EntityNotFoundException {
        ...
        if (predaj.getId() == null) { //insert
            ...
            valuesMap.put("client", predaj.getClient());
            valuesMap.put("date", predaj.getDate());
            valuesMap.put("price", predaj.getPrice());
            valuesMap.put("tour", predaj.getTour());
            return new Predaj(
            ...
            int changed = jdbcTemplate.update(sql, predaj.getClient(), predaj.getDate(), predaj.getPrice(), predaj.getTour());
            if (changed == 1) {
                return predaj;
            } else {

...
public class MysqlType_tourDao implements Type_tourDAO {
    ...
    public Type_tour getById(long id) throws EntityNotFoundException {
        try {
            return jdbcTemplate.queryForObject("SELECT type,id FROM type_tour WHERE id =" + id, new TourRowMapper());
    ...
    @Override
    public Type_tour idByName(String type) throws EntityNotFoundException {
       try {
        String sql = "SELECT * FROM type_tour WHERE type =?";
    ...
    public List<Type_tour> getAll() {
        return jdbcTemplate.query("SELECT * FROM type_tour", new TourRowMapper());
    ...
    private class TourRowMapper2 implements RowMapper<Type_tour> {
        @Override
        public Type_tour mapRow(ResultSet rs, int rowNum) throws SQLException {
            long id = rs.getLong("id");
            //   String type = rs.getString("type");
            return new Type_tour(id);
        }
